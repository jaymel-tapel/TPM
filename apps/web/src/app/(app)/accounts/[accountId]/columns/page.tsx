...
import { getAccountBoard } from "@/queries/tasks";
 ...
 * sorted into, so it sits inside that client, reachable from the page it
 * shapes and nowhere else.
 */
export default async function AccountColumnsPage({
  params,
}: {
  ...
  return (
    <>
      <PageHeader
        ...
        commands={
          <CommandBar>
            <Command icon={ArrowLeft} href={`/accounts/${accountId}/tasks`}>
              Back to tasks
            </Command>
          </CommandBar>
        }
