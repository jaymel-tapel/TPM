...
import { getAccountBoard, getBoardView } from "@/queries/tasks";
 ...
 * An account's work.
 *
 * There is no Boards section any more, and this is why: a board was never a
 * place people meant to go, it was the columns this page is drawn with. Tasks
 * is the section; **Board is a way of looking at it**, alongside List. The
 * rail used to carry both and repeated every client's name under each, which
 * is two hierarchies describing one thing.
 */
export default async function AccountTasksPage({
  ...
  const board = await getAccountBoard(accountId);
  if (!board) {
    return (
      <>
        <PageHeader eyebrow={account.name} title="Tasks" />
  ...
  const href = (next: Partial<{ list: boolean; mine: boolean }>) => {
    ...
    return `/accounts/${accountId}/tasks${search ? `?${search}` : ""}`;
  };
      ...
      <CommandBar className="mb-4">
        ...
        <div className="ml-auto flex items-center gap-1">
          {canViewAccount(user, accountId) ? (
            <>
              <Command icon={Settings2} href={`/accounts/${accountId}/columns`}>
                Columns
              </Command>
              <CommandDivider />
            </>
